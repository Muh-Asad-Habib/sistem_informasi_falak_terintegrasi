...
describe('hitungJadwalSalat — golden test terhadap contoh Modul AIK IV', () => {
  ...
  const selisihMenit = (a: string, b: string) => {
  ...
  it('memakai parameter metode yang benar-benar mempengaruhi hasil (bukan label kosong)', () => {
    ...
    const subuhLebihDini = hitungJadwalSalat(
      coordinate, tanggal, TZ_WITA, ELEVASI_MDPL, 'Muhammadiyah', 2,
      { hSubuh: -18 }
    );
    ...
    const isyaBeda = hitungJadwalSalat(
      coordinate, tanggal, TZ_WITA, ELEVASI_MDPL, 'Muhammadiyah', 2,
      { hIsya: -20 }
    );
  ...
  it('menghasilkan jadwal Markaz Unismuh yang konsisten dengan contoh modul (toleransi ±2 menit)', () => {
    ...
    const hasil = hitungJadwalSalat(
      coordinate, tanggal, TZ_WITA, ELEVASI_MDPL, 'Muhammadiyah', 2, PARAMETER_MODUL
    );
...
describe('preset multi-metode & mazhab Asar', () => {
  ...
  it('Umm al-Qura menghitung Isya sebagai selang tetap setelah Magrib, bukan dari ketinggian matahari', () => {
    ...
    expect(menit(hasil.isya) - menit(hasil.magrib)).toBe(
      PARAMETER_METODE.UmmAlQura.isyaMenitSetelahMagrib
    );
  ...
  it('menolak metode yang tidak dikenal', () => {
    expect(() =>
      ...
      hitungJadwalSalat(coordinate, tanggal, TZ_WITA, ELEVASI, 'MetodeKarangan', 2)
    ).toThrow();
  });
});
