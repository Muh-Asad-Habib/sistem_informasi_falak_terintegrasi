...
export const KAABAH_COORDINATES: Coordinate = {
 ...
 */
export function hitungSelisihBujurC(lngTempat: number): AngleResult {
 ...
 */
export function hitungArahKiblat(coordinate: Coordinate): QiblaResult {
  ...
  if (lat < -90 || lat > 90 || lng < -180 || lng > 180) {
    throw new HisabError(
      'INVALID_COORDINATES',
      `Koordinat tidak valid: Lintang harus [-90, 90] dan Bujur harus [-180, 180]. Input: Lintang ${lat}, Bujur ${lng}`
    );
  }
